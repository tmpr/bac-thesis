...
\begin{example}
	...
	\end{matrix}
	\end{equation*}
	For the first block, we would have in the first column three pairs of \texttt{AA}, and in the second column two pairs of \textt{AC} and one pair of \textt{CC} 
...
\begin{remark}
	...
	% The reason for the $log_2$ and the multiplication of 2 is of heuristic, yet somewhat intuitive nature. Observe
	% that the decision boundary for rounding numbers is at $\frac 1 2$. If we let $x = \mathbf Q_{i, j}$ and $y = 2\mathbf p_i\mathbf p_j$, this means
	% \begin{equation*}
	% 	\mathbf S_{i, j} = k \implies k + \frac 1 2 \ge 2\log_2(\frac x y) \ge k - \frac 1 2 
	% \end{equation*}
	%         implying	
	% \begin{align*}
	% 	k + \frac 1 2 \ge 2\log_2(\frac x y) &\ge k - \frac 1 2 &\iff
	% 	\\ 2k + 1 \ge \log_2(\frac x y) &= 2k - 1 &\iff
	% 	\\  2^{2k + 1} \ge \frac x y &\ge 2^{2k - 1}&\iff
	% 	\\  2^{2k +1}y \ge &\ge 2^{2k - 1}y.
	% \end{align*}
	% So, for example, we can say that if the value between \texttt{A} and \textt{C} is 2, then a pairing of the 
	% two was observed between 8 to 32 times as often as purely by chance. 
	% The reason for the $log_2$ and the multiplication of 2 is of heuristic, yet somewhat intuitive nature. Observe
	% 	that the decision boundary for rounding numbers is at $\frac 1 2$. If we let $x = \mathbf Q_{i, j}$ and $y = 2\mathbf p_i\mathbf p_j$, this means
	% 	\begin{equation*}
	% 		\mathbf S_{i, j} = k \implies k + \frac 1 2 \ge 2\log_2(\frac x y) \ge k - \frac 1 2 
